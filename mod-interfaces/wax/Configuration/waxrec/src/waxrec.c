...
WaxPacket *parseWaxPacket(const void *inputBuffer, size_t len, unsigned long long now)
{
	...
	if (len >= 12 && buffer[0] == 0x12 && buffer[1] == 0x78)
	{
		...
		*/
        ...
        if (len < expectedLength) { fprintf(stderr, "WARNING: Ignoring truncated- or unknown-format data packet (received %d expected %d).", len, expectedLength); }
        else
        {
			int i;
...
#endif
		flags |= (initString == NULL) ? O_RDONLY : O_RDWR;
